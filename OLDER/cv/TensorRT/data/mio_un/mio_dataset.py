...
np_features = np.zeros((NUM_TRAINING_IMAGES, NUM_CHANNELS, IMG_HEIGHT, IMG_WIDTH), np.uint8)
np_labels = np.zeros((NUM_TRAINING_IMAGES,1),np.uint64)
np_test_features = np.zeros((NUM_TEST_IMAGES, NUM_CHANNELS, IMG_HEIGHT, IMG_WIDTH), np.uint8)
...
for c in classes:
    ...
    for i in range(0,NUM_TRAINING_IMAGES_IN_CLASS):
        ...
        np_img_reshape = np.zeros((NUM_CHANNELS, IMG_HEIGHT, IMG_WIDTH), dtype=np.uint8)
        ...
        ind+=1
    cid+=1
np.save('train_images.npy', np_features)
np.save('train_labels.npy', np_labels)
