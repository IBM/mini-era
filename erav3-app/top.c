...
void top(uint8_t* out_msg, unsigned in_msg_len, unsigned num_in_data, fx_pt *in_data) // , const unsigned conf_info_size)
{
  printf("In the top routine...\n"); //  with info_size %u and n_chunks = %u\n", conf_info_size, n_chunks);
  printf("Calling compute\n");
  compute(in_msg_len, num_in_data, in_data, out_msg); // outbuff);
  DEBUG(printf("CMP_MSG:\n%s\n", out_msg));
}


void do_from_top(unsigned in_msg_len, unsigned num_inputs, float *in_real, float* in_imag, unsigned * num_msg_chars, uint8_t* out_msg) // , const unsigned conf_info_size)
{
  DEBUG(printf("In the do_from_top routine with %u inputs\n", num_inputs));
  for (int i = 0; i < num_inputs; i++) {
    input_data[i] = in_real[i] + I * in_imag[i];
  }
  DEBUG(printf("Calling compute\n"));
  compute(in_msg_len, num_inputs, input_data, out_msg); // outbuff);
  *num_msg_chars = 1500;
...
//void compute(fx_pt *inbuff, fx_pt1 *outbuff) {
