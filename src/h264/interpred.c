...
)
{
  ...
  unsigned char dflag = 0; // Just some random initialization?
  int i,j;
    ...
    unsigned flag)
{
  ...
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=mv complete dim=1)
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=mv complete dim=2)
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=mv complete dim=3)
...
//ON_HLS(#pragma HLS PIPELINE)
  ...
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=temp complete dim=1)
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=temp complete dim=2)
    ...
    unsigned char flag)
{
  ...
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=mv0 complete dim=1)
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=mv0 complete dim=2)
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=mv0 complete dim=3)

  ON_HLS(#pragma HLS ARRAY_PARTITION variable=mv1 complete dim=1)
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=mv1 complete dim=2)
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=mv1 complete dim=3)
//ON_HLS(#pragma HLS PIPELINE)
  int i,j;
  int x,y;
  int x0,y0;
  ...
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=temp0 complete dim=1)
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=temp0 complete dim=2)
  unsigned char temp1[3][3];
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=temp1 complete dim=1)
  ON_HLS(#pragma HLS ARRAY_PARTITION variable=temp1 complete dim=2)
