...
bool SampleINT8::build(DataType dataType)
{
    ...
    // NC -- this is to generate the calibration table anyway, irrespective of INT8 support on my machine 
    ...
    SampleUniquePtr<nvcaffeparser1::ICaffeParser>& parser, DataType dataType)
{
    ...
    if (dataType == DataType::kINT8)
    {
        MIOBatchStream calibrationStream(mParams.calBatchSize, mParams.nbCalBatches, "mio_training_images.idx",
            "mio_training_labels.idx", mParams.dataDirs);
        calibrator.reset(new Int8EntropyCalibrator2<MIOBatchStream>(
...
bool SampleINT8::infer(std::pair<float, float>& score, int firstScoreBatch, int nbScoreBatches)
{
    ...
    MIOBatchStream batchStream(
        mParams.batchSize, nbScoreBatches + firstScoreBatch, "mio_training_images.idx", "mio_training_labels.idx", mParams.dataDirs);
...
SampleINT8Params initializeSampleParams(const samplesCommon::Args& args, int batchSize)
{
    ...
    params.prototxtFileName = "mio.prototxt";
    params.weightsFileName = "mio.caffemodel";
    params.networkName = "mio";
    return params;
}
