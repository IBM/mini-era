...
IMG_HEIGHT =28
NUM_CHANNELS=1
NUM_TRAINING_IMAGES_IN_CLASS = 5000  ## choose 25k images per class as of now
...
np_labels = np.zeros((NUM_TRAINING_IMAGES,1),np.uint8)
np_test_features = np.zeros((NUM_TEST_IMAGES, NUM_CHANNELS, IMG_HEIGHT, IMG_WIDTH), np.float32)
np_test_labels = np.zeros((NUM_TEST_IMAGES,1),np.uint8)
