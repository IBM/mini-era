...
class MIOBatchStream : public IBatchStream
{
public:
    MIOBatchStream(int batchSize, int maxBatches, const std::string& dataFile, const std::string& labelsFile,
        const std::vector<std::string>& directories)
        : mBatchSize{batchSize}
        , mMaxBatches{maxBatches}
        , mDims{3, 1, 28, 28} //!< We already know the dimensions of MIO images.
    {
        readDataFile(locateFile(dataFile, directories));
        readLabelsFile(locateFile(labelsFile, directories));
    }
...
private:
    void readDataFile(const std::string& dataFilePath)
    {
        ...
        int magicNumber, numImages, channels, imageH, imageW;
        file.read(reinterpret_cast<char*>(&magicNumber), sizeof(magicNumber));
        // All values in the MIO files are big endian.
        magicNumber = samplesCommon::swapEndianness(magicNumber);
        assert(magicNumber == 3332 && "Magic Number does not match the expected value for an MIO image set");
        ...
        numImages = samplesCommon::swapEndianness(numImages);
        channels = samplesCommon::swapEndianness(channels);
        imageH = samplesCommon::swapEndianness(imageH);
        imageW = samplesCommon::swapEndianness(imageW);

        // The MIO data is made up of sinlge-precision floating point, so we do not need to cast to float and normalize.
        int numElements = numImages * channels * imageH * imageW;
        std::vector<float> rawData(numElements);
        file.read(reinterpret_cast<char*>(rawData.data()), numElements * sizeof(float));
        mData.resize(numElements);
    }
    ...
    void readLabelsFile(const std::string& labelsFilePath)
    {
        ...
        // All values in the MIO files are big endian.
        magicNumber = samplesCommon::swapEndianness(magicNumber);
        assert(magicNumber == 2050 && "Magic Number does not match the expected value for an MIO labels file");
